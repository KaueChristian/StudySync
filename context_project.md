...
| 4 | 🚫→✅ **Corrigido** | **Sessão pendente que já passou desaparece do painel.** O card rotulado "Sessões pendentes" (`DashboardPage.jsx:200`) exibia `sessions_upcoming`, que só conta `start_at >= agora` (`dashboard.py`). Não existia nenhum contador de "atrasada" — e o docstring de `scheduler.py:9` ainda prometia um job `mark_overdue_sessions` que não existia. Corrigido em 2026-09-12: `DashboardStats` agora computa e entrega `sessions_pending` (todas as pendentes) e `sessions_overdue` (`start_at < now`). O painel exibe o total pendente e destaca sessões em atraso no hint. Docstring de `scheduler.py` corrigida. | Testado na conta demo ao vivo: 3 sessões pendentes de agosto exibidas como "3 pendentes" e "3 em atraso" (antes exibia "0"). |
...
| 13 | 🚫→✅ **Corrigido** | `notifications.schedule_id` sem integridade referencial: excluir a sessão deixava o ID órfão e clicar na notificação falhava em silêncio. Corrigido em 2026-09-12: `delete_schedule` atualiza `schedule_id = NULL` nas notificações; adicionado `ForeignKey(..., ondelete="SET NULL")` no modelo; e `SchedulePage.jsx` busca a sessão diretamente ou avisa o usuário com toast informativo. | Validado no banco de dados e no frontend. |
